...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Kirim notifikasi ke Creative Director untuk task kategori produksi
        if ($categoryId === $produksiCategoryId) {
            ...
            foreach ($creativeDirectors as $cd) {
                ...
                sendNotification($cd['id'], "Ada task baru dari Tim Redaksi yang memerlukan persetujuan", "view_task.php?id=$taskId");
                
                // Kirim notifikasi WhatsApp jika ada nomor WhatsApp
                if (!empty($cd['whatsapp_number'])) {
                    // Format pesan WhatsApp
                    $message = "Halo *{$cd['name']}*,\n\n";
                    $message .= "Ada task baru dari Tim Redaksi yang memerlukan persetujuan Kamu:\n\n";
                    $message .= "*Judul:*\n{$title}\n\n";
