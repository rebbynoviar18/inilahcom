...
$stmt = $pdo->prepare("
    SELECT t.*, 
           ...
           tsl.notes as rejection_reason,
           tsl.timestamp as rejection_date,
    ...
    LEFT JOIN (
        SELECT task_id, notes, timestamp, updated_by
        FROM task_status_logs
        WHERE status = 'rejected'
        ORDER BY timestamp DESC
        LIMIT 1
    ) tsl ON t.id = tsl.task_id
    LEFT JOIN users u ON tsl.updated_by = u.id
    WHERE t.id = ? AND t.created_by = ? AND t.status = 'rejected'
");
$stmt->execute([$taskId, $userId]);
$task = $stmt->fetch();

if (!$task) {
    $_SESSION['error'] = "Task tidak ditemukan atau tidak dapat direvisi";
    header('Location: tasks.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        // Update task dan ubah status menjadi waiting_head_confirmation
        $stmt = $pdo->prepare("
            UPDATE tasks SET
                title = ?,
                description = ?,
            ...
            WHERE id = ? AND created_by = ? AND status = 'rejected'
        ");
        ...
        $stmt = $pdo->prepare("INSERT INTO task_status_logs (task_id, status, updated_by, notes) VALUES (?, 'waiting_head_confirmation', ?, 'Task direvisi setelah ditolak')");
        $stmt->execute([$taskId, $userId]);
        ...
        if ($directorId) {
            ...
            $stmt->execute([
                                $directorId,
                "Task yang ditolak telah direvisi dan memerlukan persetujuan",
                "view_task.php?id=$taskId"
            ]);
        }
        
        $_SESSION['success'] = "Task berhasil direvisi dan dikirim kembali untuk persetujuan";
        header("Location: view_task.php?id=$taskId");
        exit();
        
    } catch (Exception $e) {
        $_SESSION['error'] = "Gagal merevisi task: " . $e->getMessage();
    }
}

$pageTitle = "Revisi Task Ditolak";
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="alert alert-danger mb-4">
                        ...
                        <p><strong>Alasan Penolakan:</strong> <?= nl2br(htmlspecialchars($task['rejection_reason'])) ?></p>
                        <p class="mb-0"><small>Ditolak oleh <?= htmlspecialchars($task['rejected_by_name']) ?> pada <?= date('d M Y H:i', strtotime($task['rejection_date'])) ?></small></p>
                    </div>
